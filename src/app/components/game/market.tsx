...
export function MarketTicker({ market }: { market: MarketView }) {
  ...
  return (
    <div className="rounded-xl border border-accent/30 bg-card/80 py-2.5 flex items-center justify-center divide-x divide-border">
  ...
  return (
    ...
    >
      ...
      <span className="text-foreground">
        ...
        <span className={order.side === 'buy' ? 'text-primary' : 'text-accent'}>
          {order.price}
        </span>
      </span>
      {mine && onCancel ? (
...
export function TradesFeed({ trades, youId, max = 8 }: { trades: Trade[]; youId?: string; max?: number }) {
  if (trades.length === 0) {
