...
describe('helpers', () => {
  it('tradedNet, tradedCash and openSellRemaining track positions and cash', () => {
    const orders: Order[] = []
    matchOrder(orders, order('P1', 'sell', 30, 12), makeTradeId)
    ...
    // 20 credits @ 12 = 240: P2 spent it, P1 earned it.
    expect(tradedCash(trades, 'P2')).toEqual({ buyCash: 240, sellCash: 0 })
    expect(tradedCash(trades, 'P1')).toEqual({ buyCash: 0, sellCash: 240 })
